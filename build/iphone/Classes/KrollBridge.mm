...
@implementation comcouchcontrolmodel1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_comcouchcontrolmodel1);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any comcouchcontrolmodel1 patches in JS before a context is loaded... nice for 
...
{
	// create comcouchcontrolmodel1 global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_comcouchcontrolmodel1 = [[comcouchcontrolmodel1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_comcouchcontrolmodel1];
	
	NSString *_comcouchcontrolmodel1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _comcouchcontrolmodel1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_comcouchcontrolmodel1 valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.comcouchcontrolmodel1.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
